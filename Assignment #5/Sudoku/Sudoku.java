...
class Sudoku
{
    ...
    boolean Cleared[][];		// keep track of which values have been cleared by minimization procedures
    boolean Prefilled[][];		// keep track of which values were given from the start
    ...
    public boolean backtrack(int x, int y) {  	
		...
		if (Prefilled[x][y]) {
			...
			// try all possible values    TODO try only values in Potential[x][y][i]
	    	for (int i = 1; i <= N; i++) {
	    		Grid[x][y] = i;
	    		
	    		if (isValid()) {				// try new value
